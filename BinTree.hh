...
void BinTree<T>::readVisualFormatRec(const vector<string> &v, int i, int j, BinTree<T> &t)
{
  ...
  int jini = j;
  while (jini > 0 and line[jini - 1] != ' ')
    jini--;
  int jend = j;
  while (jend + 1 <= int(line.size()) - 1 and line[jend + 1] != ' ')
    jend++;
  ...
  int jdown = jini;
  while (jdown < int(linedown.size()) and jdown <= jend and linedown[jdown] == ' ')
    jdown++;
  ...
  {
    while (jleft >= 0 and line2down[jleft] == '-')
      jleft--;
  ...
  {
    while (jright >= 0 and jright < int(line2down.size()) and line2down[jright] == '-')
      jright++;
...
vector<string> BinTree<T>::generateVisualFormatRec(const BinTree<T> &t)
{
  ...
  if ((rightrootpos + addspace - leftrootpos) % 2 == 1)
    addspace++;
  if (addspace > 0)
  {
